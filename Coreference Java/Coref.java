...
* @Author Doris Zhou
* @Author Jack Hester
* This program resolves co-references within the user provided file(s)
* Neural, deterministic, and statistical methods of coreference resolution are supported
* It relies on Stanford CoreNLP 
* When written, Stanford CoreNLP version 3.9.2 was used
*/
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            if ("-inputDir".equals(args[i])) {
                ...
                i++;
            } else if("-approach".equals(args[i])) {
                approach = args[i + 1];
                i++;
            }
        }
        ...
        //choose the approach system for coref based on user arg (deterministic, neural, or statistical)
        System.out.println("Using the "+approach+" approach for coreference resolution...");
        if (approach.equals("deterministic")){
            props.setProperty("annotators", "tokenize,ssplit,pos,lemma,ner,parse,dcoref");
        }
        else{
            props.setProperty("annotators", "tokenize,ssplit,pos,lemma,ner,parse,coref.mention,coref");
            if (approach.equals("neural")){
                props.setProperty("coref.algorithm","neural");
            }
            else if(approach.equals("statistical")){
                props.setProperty("coref.algorithm","statistical");
            }
        }
        props.setProperty("quiet", "true"); //suppress unnecessary server output that slows process
        //props.setProperty("coref.algorithm", "neural");

        RedwoodConfiguration.current().clear().apply(); //mute server output
